...
- 五个角色：`planner` / `coder` / `verifier` / `summarizer` / `dedup`
  - planner / coder / summarizer / dedup 都是 LLM 驱动的 LLMNode 实例
    ...
    - summarizer 用 `ROLE_CONFIGS["summarizer"]["model"]`(默认 `gpt-4o-mini`)，每个 passed case 末尾跑一次,从全 task trace 提炼 1-2 条 project-level 经验
    - dedup 用 `ROLE_CONFIGS["dedup"]["model"]`(默认 `gpt-4o-mini`),merge facts 时判语义等价
...
**1. runner 入口**（[runners/mbpp/task.py](runners/mbpp/task.py)）
...
│  · tool_pool/ 委托给 env 干物理活(read 真去 open,run 真去    │
...
| **tool_pool** | 翻译 / 包装层 | LLM 工具(ops.py: schema in/string out)+ Python helper(test_runner.py: 包 env 的结果成 TestRunResult);不直接做 subprocess |
...
读起来一句话："**engine 调度,planner / coder 想,verifier 验,summarizer 总结,dedup 判等价,tool_pool 翻译,env 干活,memory 旁观记录**"。
...
Roles (cognitive — 都有 PROMPT,被 LLMNode 包装):
  role_pool/planner.py     用 LLM 出 plan
  role_pool/coder.py       用 LLM 写代码 / 调 tools
  role_pool/verifier.py    (没 LLM,纯 pytest 函数)
  role_pool/summarizer.py  用 LLM 提炼 1-2 条 project-level 经验(passed case 末尾)
  role_pool/dedup.py       用 LLM 判 fact 语义等价(merge 时)
...
Tools (callable utilities pool):
  tool_pool/__init__.py    TOOL_DEFINITIONS + execute_tool dispatcher (LLM-facing)
  tool_pool/ops.py         LLM-facing tools: read_file / write_file / list_dir /
                           search_in_files / replace_in_file / run_command
  tool_pool/test_runner.py Python helper: test command discovery + result wrapper
                           (used by verifier; NOT exposed to LLM dispatcher)

Runners (CLI 入口):
  runners/mbpp/task.py        MBPP 主入口(setup / run / all)
  runners/mbpp/html.py        生成 dataset.html(被 mbpp.task run 自动调用,
                              也可单独 `python -m runners.mbpp.html` 重渲染)
  runners/story_task/         (空目录,等 Track A / B 落地后填进去)

Config:
  config.py         所有可调实验参数(MODEL / MAX_STEPS / MAX_REPLANS /
                    ROLE_CONFIGS / MAX_MEMORY_FACTS 等)
```
...
### 2.5 Per-role 超参数：`ROLE_CONFIGS`

[`config.py`](config.py) 里有一个 `ROLE_CONFIGS` 字典，是**所有 per-role 实验 knob 的单一来源**。每个 role 当前 5 个参数：

```python
ROLE_CONFIGS = {
    "planner":    {"model": "openai/gpt-4o-mini", "max_steps": 1,    "uses_tools": False, "temperature": 0.2, "max_tokens": None},
    "coder":      {"model": "openai/gpt-4o-mini", "max_steps": 8,    "uses_tools": True,  "temperature": 0.1, "max_tokens": None},
    "verifier":   {"model": None,                 "max_steps": None, "uses_tools": False, "temperature": None,"max_tokens": None},
    "summarizer": {"model": "openai/gpt-4o-mini", "max_steps": 1,    "uses_tools": False, "temperature": 0.4, "max_tokens": None},
    "dedup":      {"model": "openai/gpt-4o-mini", "max_steps": 1,    "uses_tools": False, "temperature": 0.0, "max_tokens": None},
}
```

| 字段 | 含义 |
|---|---|
| `model` | OpenRouter model id；`None` = 这个 role 不调 LLM（只 verifier 是这样） |
| `max_steps` | LLMNode loop 最多跑几圈（planner/summarizer/dedup 一次性 1，coder 多轮 8） |
| `uses_tools` | 是否给该 role 配 `tool_pool/ops.py` 全套工具（只 coder 需要） |
| `temperature` | 采样温度。tuning 出发点：planner/coder 低（0.1-0.2 求稳），summarizer 中（0.4 措辞自然），dedup 零（二元判断绝对一致） |
| `max_tokens` | 输出长度上限；`None` 用 API 默认 |

**为什么用 dict 而非扁平常量**：
- 加新 role = 加一行字典条目（不动 engine.build_llm_nodes）
- 加新参数（比如未来加 `enable_thinking`）= 给所有 role 加一个 key + llm.chat 透传
- 实验时全局换模型：一个 dict comprehension 搞定（`for role in ROLE_CONFIGS: ROLE_CONFIGS[role]["model"] = "anthropic/claude-sonnet-4"`）

`engine.build_llm_nodes()` 直接迭代 `ROLE_CONFIGS` 构造每个 LLMNode：data-driven，零 if-else 分支。
...
这个 context 喂回 planner 的 prompt（[role_pool/planner.py](role_pool/planner.py) 的 `build_input` 第二段就是 `Previous Attempt Failed`），让它换思路出新 plan。
...
(runner 在 batch 边界)
        ...
        → dedup_node(gpt-4o-mini)逐条流式判等价
...
2. **跑完后(主线程，单线程)**：runner 构造一个 dedup LLMNode（用 [`config.ROLE_CONFIGS["dedup"]["model"]`](config.py)，默认 `openai/gpt-4o-mini`），调 `memory.merge_facts_into_global(local_files, target, dedup_node=...)` 扫描所有 case 的 `long_term_memory.json`，**每条新 fact 让 LLM 判断和池里已有 fact 是否语义等价**：等价就累加 `reinforce_count` / `confidence` / 合并 `reinforced_by`；不等价就当 novel 加进池。最后写一次 `mbpp_global_facts.json`。详见 [`role_pool/dedup.py`](role_pool/dedup.py) 的 PROMPT 设计。

**为什么不用字符串 normalize 做 dedup**：之前用过 lowercase + 折叠空白的归一化，实测在 baseline 822 条 raw fact 里只抓到 42 条重复（5%）。绝大多数重复是"同义改写"——`"uses pytest -q"` vs `"tests run quietly via pytest -q"` 这种字面差异大但语义相同的，字符串 normalize 完全无能为力。所以现在直接用 LLM 判，gpt-4o-mini 一次 ~$0.0002，跑全 baseline 也就 ~$0.30。
...
`mbpp_task` 是主入口，3 个子命令通过 `python -m runners.mbpp.task <子命令>` 调用：

| 命令 | 干什么 |
|---|---|
| `setup --exp NAME [--subset sanitized\|full] [--split test\|train\|...] [--limit N]` | 从 HuggingFace 下载 MBPP，物化 N 个 case 到 `single_case_details/` |
| `run --exp NAME [--limit N] [--workers N] [--skip-existing]` | 跑 agent 过所有(或前 N 个)物化的 case，写报告 + 生成 dataset.html |
...
| 命令 | 干什么 |
|---|---|
| `python -m runners.mbpp.html --exp NAME` | 单独重渲染 dataset.html(`mbpp_task run` 跑完会自动调一次，改了模板想重渲染就手动跑这个) |
...
```bash
# 第一次跑挂在第 187 题
python -m runners.mbpp.task all --exp baseline --split test --limit 0 --workers 4

# 重跑,跳过 1-187,从 188 开始
python -m runners.mbpp.task run --exp baseline --skip-existing --workers 4
```
...
python -m runners.mbpp.task all --exp smoke --limit 10

# 全量 sanitized(427 题)
python -m runners.mbpp.task all --exp full_baseline --limit 0 --split test

# 续跑:第一次跑挂了，跳过已完成的 case 接着跑
python -m runners.mbpp.task run --exp full_baseline --skip-existing --workers 4

# 单独重渲染 HTML(`run` 跑完会自动调一次，模板改了想刷新时再手动跑)
python -m runners.mbpp.html --exp full_baseline
```
...
> 想做不同 pipeline 对比实验时，单独写脚本组合 `setup` / `run` 调用即可，主 CLI 不内嵌 ablation framework。
...
`Environment` 是个有状态对象，封装 `workspace` + 所有物理操作（read/write/list/walk/run_command）。**`tool_pool/ops.py` 和 `tool_pool/test_runner.py` 都通过 env.run_command 跑命令**，全代码库只有一处 `subprocess.run` 调用（在 `environment.py`）。

好处：
- 将来上 docker：实现 `DockerEnvironment(Environment)`，覆盖 `run_command` / `read_file` 等方法。tool_pool / verifier 一行不动
...
```bash
# 物化一次
python -m runners.mbpp.task setup --exp dataset_v1 --limit 50

# 跑实验 A
python -m runners.mbpp.task run --exp dataset_v1     # 用默认配置

# 修改 prompt / model / 任何 config，开新实验
cp -r Execution/dataset_v1 Execution/exp_a_new_prompt
python -m runners.mbpp.task run --exp exp_a_new_prompt
```
...
**修法**:删掉 coder 的 save_memory tool,新增独立的 [`role_pool/summarizer.py`](role_pool/summarizer.py) role。summarizer 在 `overall_passed=True` 之后被 engine 调一次,看到 case 的全 trace(prompt + plan + 动作时间线 + 最终代码),用全系统视角(不只 planner 也不只 coder)输出 1-2 条 project-level 经验。**作者就是读者**(都是高层视角),消除视角错配;频率从 ~5/case 降到 1-2/case。
...
**跨 case (batch merge 时)**:`memory.add_facts_to_pool` 调 `dedup_node`(gpt-4o-mini)做语义判等价,能抓到 "uses pytest -q" vs "tests run quietly via pytest -q" 这种 paraphrase。这是主要的去重战场。
...
| ~~Async 批量~~ ✓ 已实现 | `runners/mbpp/task.py` `cmd_run` `ThreadPoolExecutor` | MBPP 257 题 4 worker ~3.5x 加速 |
| **Docker sandbox** | `environment.py` + `tool_pool/test_runner.py` | SWE-bench 必需；新增 `DockerRunner` backend，env 加 docker exec 路径 |
| **Single-loop role** | 多写一个 `system_prompt` + 一个 `build_llm_nodes` 变体 | 跟 plan→verify 多角色对比，做消融 |
| **Runner 抽象层** | 提取 `runners/_base.py`（common run_one/cmd_run/_exp_paths） | 加 SWE-bench / HumanEval 时不需要复制 `runners/mbpp/task.py` 的样板代码 |
